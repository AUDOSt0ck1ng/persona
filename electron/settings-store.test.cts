...
function writeEmptyPackagedLibrary(root: string): string {
...
function fixture(context: TestContext): SettingsFixture {
...
function writeGlb(filePath: string): void {
...
function writePackagedLibrary(root: string): string {
  const packagedLibraryPath = path.join(root, "library.json");
  fs.writeFileSync(
    packagedLibraryPath,
    JSON.stringify({
...
test("imports, persists, resolves, and deletes user assets", (context) => {
  ...
  snapshot = store.addAnimationClips(animationId, [sourceAnimation]);
  ...
  const animationUrl = animation.asset_urls[0];
  assert.ok(animationUrl);
  assert.match(animationUrl, /^persona-asset:\/\/animation\//);
  const storedAnimation = store.resolveAssetRequest(animationUrl);
  if (typeof storedAnimation !== 'string') {
    throw new Error('Expected the stored animation to resolve to a file path.');
  }
  ...
  snapshot = store.deleteAnimation(animationId);
  assert.equal(
...
test("holds a hub-sourced model in memory only, never in settings.json", (context) => {
  ...
  const persisted: unknown = JSON.parse(fs.readFileSync(settingsPath, "utf8"));
  assert.ok(isRecord(persisted));
  assert.equal(persisted.default_model_id, localModel.id);
  assert.ok(Array.isArray(persisted.models));
  assert.equal(
    persisted.models.some(
      (model: unknown) => isRecord(model) && model.id === hubModel.id,
    ),
    false,
  );
...
test("uses copy-on-write packaged edits and resets only that layer", (context) => {
  ...
  const modifiedAnimation = snapshot.animations.find(
    (animation) => animation.id === original.id,
  );
  assert.ok(modifiedAnimation);
  assert.equal(modifiedAnimation.modified, true);
  ...
  const restoredAnimation = snapshot.animations.find(
    (animation) => animation.id === original.id,
  );
  assert.ok(restoredAnimation);
  assert.equal(restoredAnimation.animation_description, original.animation_description);
  ...
  const reloadedAnimation = reloaded.animations.find(
    (animation) => animation.id === original.id,
  );
  assert.ok(reloadedAnimation);
  assert.equal(reloadedAnimation.animation_description, original.animation_description);
});
...
test("stores complete independent lighting profiles and enforces UI ranges", (context) => {
  ...
  const configuredLighting = snapshot.model_lighting["configured-model"];
  assert.ok(configuredLighting);
  assert.equal(configuredLighting.environment_intensity, 0.35);
...
test("groups multiple uploaded clips under one action and removes them independently", (context) => {
  ...
  const createdAction = snapshot.animations.find(
  ...
  const removedClip = action.clips[0];
  assert.ok(removedClip);
  const removedPath = store.resolveAssetRequest(removedClip.asset_url);
